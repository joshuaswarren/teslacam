...
const {
  BACKUP_DIR, DROPBOX_UPLOADER, LOCK_FILE_NAME, WAIT_INTERVAL, DELETE_ON_UPLOAD, RSYNC_UPLOAD_DESTINATION
...
//    execSync(`${DROPBOX_UPLOADER} -s upload ${filename} .`, { bubbleError: true, noop: opts.noop });
      var rsync = new Rsync()
        ...
        .flags('az')
        .source(`${filename}`)
        .destination(RSYNC_UPLOAD_DESTINATION);
    ...
    console.log(`Uploaded ${filename} via rsync`);
    ...
    console.log(`Failed to upload ${filename} via rsync`);
    return false;
  }
};
